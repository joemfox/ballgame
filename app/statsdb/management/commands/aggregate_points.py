...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for player in players:
            print(f'{player}')
            agg = models.BattingStatLine.objects.filter(player=player, date__year=year).filter(Q(game_type='R')).aggregate(games=Count("ab"), outs=Sum("outs"), bb=Sum("bb"), triples=Sum("triples"), h=Sum("h"), cycle=Count(Case(When(cycle=True,then=1))), doubles=Sum("doubles"), outfield_assists=Sum("outfield_assists"), cs=Sum("cs"), e=Sum("e"), gidp=Sum("gidp"), hr=Sum("hr"), r=Sum("r"), lob=Sum("lob"), po=Sum("po"), rl2o=Sum("rl2o"), rbi=Sum("rbi"), k_looking=Sum("k_looking"), k=Sum("k"), sb=Sum("sb"), sombrero=Count(Case(When(sombrero=True,then=1))), FAN_outs = Sum("FAN_outs"), FAN_bb = Sum("FAN_bb"), FAN_triples = Sum("FAN_triples"), FAN_h = Sum("FAN_h"), FAN_cycle = Sum("FAN_cycle"), FAN_doubles = Sum("FAN_doubles"), FAN_outfield_assists = Sum("FAN_outfield_assists"), FAN_cs = Sum("FAN_cs"), FAN_e = Sum("FAN_e"), FAN_gidp = Sum("FAN_gidp"), FAN_hr = Sum("FAN_hr"), FAN_r = Sum("FAN_r"), FAN_lob = Sum("FAN_lob"), FAN_po = Sum("FAN_po"), FAN_rl2o = Sum("FAN_rl2o"), FAN_rbi = Sum("FAN_rbi"), FAN_k_looking = Sum("FAN_k_looking"), FAN_k = Sum("FAN_k"), FAN_sb = Sum("FAN_sb"))
        ...
        for player in players:
            print(f'{player}')
            agg = models.PitchingStatLine.objects.filter(player=player, date__year=year).filter(Q(game_type='R')).aggregate(
                games=Count("bf"), 
                bf=Sum('bf'), 
                ip=Sum("ip"), 
                bb=Sum("bb"), 
