...
class Command(BaseCommand):
    ...
    def load_game(self, date, game_id, overwrite, game_type, status=""):
        ...
        for batter in batters:
            ...
            statline.gidp = len([p for p in batting_plays if p['result'].get('eventType') == 'grounded_into_double_play'])
        ...
        for pitcher in pitchers:
            ...
            setattr(statline,'hb',len([p for p in pitching_plays if p['result'].get('eventType') == 'hit_by_pitch']))
            setattr(statline,'dpi',len([p for p in pitching_plays if 'double_play' in (p['result'].get('eventType') or '')]))
